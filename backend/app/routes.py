...
from .workflows.one_person_company.workflow import build_one_person_company_graph, run_one_person_company
...
def get_workflow_graph(workflow_id: str) -> WorkflowGraph:
    ...
    if workflow.type == "one_person_company":
        return build_one_person_company_graph(workflow, agents)
...
def _dispatch_run(
    ...
    if workflow.type == "one_person_company":
        return run_one_person_company(store, workflow, user_input, history=history, on_event=on_event)
    raise HTTPException(status_code=400, detail=f"Unsupported workflow type: {workflow.type}")
